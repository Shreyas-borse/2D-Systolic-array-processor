...
class BasicBlock_project(nn.Module):
    expansion=1

    def __init__(self, inplanes, planes, stride=1, downsample=None, float=False):
        super(BasicBlock_project, self).__init__()
        if float:
            self.conv1 = conv3x3(inplanes, planes, stride)
            self.conv2 = conv3x3(planes, planes)
        else:
...
class ResNet_Cifar_project(nn.Module):

    def __init__(self, block, layers, num_classes=10, float=False):
        super(ResNet_Cifar_project, self).__init__()
        #self.inplanes = 16
        self.inplanes = 8
        #self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        #self.bn1 = nn.BatchNorm2d(16)
        ...
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer_1(block, 8, layers[0], float=float)
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2, float=float)
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2, float=float)
        self.avgpool = nn.AvgPool2d(8, stride=1)
        self.fc = nn.Linear(64 * block.expansion, num_classes)
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data.fill_(1)
                m.bias.data.zero_()
    def _make_layer_1(self, block, planes, blocks, stride=1, float=False):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                QuantConv2d(self.inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False)
        ...
        layers = []
        layers.append(BasicBlock_project(8, 8, stride, None, float=float))
        downsample = nn.Sequential(QuantConv2d(8, 16, kernel_size=1, stride=stride, bias=False),nn.BatchNorm2d(16))
        layers.append(block(8, 16, stride,downsample, float=float))
        downsample = None                      
        layers.append(block(16, 16, stride, downsample, float=float))
        self.inplanes = 16
        return nn.Sequential(*layers)
...
def resnet20_quant_project(**kwargs):
    model = ResNet_Cifar_project(BasicBlock, [3, 3, 3], **kwargs)
    return model
